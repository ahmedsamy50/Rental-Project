jQuery(document).ready(function() {
   ...
   jQuery('.panel .panel-close').click(function() {
    ...
    // Minimize Panel
   $(".comment-box .panel").css({ 'padding': "10px" });
   jQuery('.panel .panel-minimize').click(function(){
      var t = jQuery(this);
      var p = t.closest('.panel');
      if(!jQuery(this).hasClass('maximize')) {
          p.find('.panel-body, .panel-footer, .comment-content').slideUp(500);
         ...
         jQuery(this).attr('data-original-title','Maximize Panel').tooltip();
          ...
          p.find('.panel-body, .panel-footer, .comment-content').slideDown(500);
          $(".comment-box .panel").css({ 'padding': "10px" });
